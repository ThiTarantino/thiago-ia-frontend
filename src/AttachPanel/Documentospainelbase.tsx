...
// ─────────────────────────────────────────────────────────────────────────────
// DocumentosPainelBase — versão padronizada/reutilizável do DocumentosPainel.
// A UI, o CSS e o comportamento são os mesmos para todos os chats; o que muda
// é o conteúdo (docs), passado por prop por cada chat (ex: documentosMae.ts).
//
// Suporta o mesmo recurso de "zip protegido por senha" do original: um doc
// com tipo "zip" pede uma senha (doc.senhaZip) e, se acertar, revela os
// arquivos internos (doc.arquivosInternos).
// ─────────────────────────────────────────────────────────────────────────────

export type ZipInnerFile = {
  id: string;
  nome: string;
  tipo: "txt" | "png";
  conteudo: string;
  meta?: string; // usado como background (gradiente) quando tipo === "png"
};
...
export type DocItem = {
  ...
  tipo: "png" | "c" | "pdf" | "zip";
  conteudo: string;
  // Só usados quando tipo === "zip":
  senhaZip?: string;
  arquivosInternos?: ZipInnerFile[];
};
...
export default function DocumentosPainelBase({ onClose, docs, titulo = "Documentos" }: Props) {
  const [docSelecionado, setDocSelecionado] = useState<DocItem | null>(null);
  const [exibirPromptSenha, setExibirPromptSenha] = useState(false);
  const [zipAberto, setZipAberto] = useState<DocItem | null>(null);
  const [subDocSelecionado, setSubDocSelecionado] = useState<ZipInnerFile | null>(null);
  const [zipPendente, setZipPendente] = useState<DocItem | null>(null);

  const [inputSenha, setInputSenha] = useState("");
  const [erroSenha, setErroSenha] = useState(false);

  const handleAbrirDoc = (doc: DocItem) => {
    if (doc.tipo === "zip") {
      setZipPendente(doc);
      setInputSenha("");
      setErroSenha(false);
  ...
  const handleVerificarSenha = (e: React.FormEvent) => {
    e.preventDefault();
    if (zipPendente && inputSenha === zipPendente.senhaZip) {
      setExibirPromptSenha(false);
      setZipAberto(zipPendente); // Libera a entrada para a pasta interna
      setErroSenha(false);
    } else {
      setErroSenha(true);
    }
    ...
    <div className="wa-doc-screen">
      ...
      <div className="wa-doc-header">
        <div className="wa-doc-header-left">
          <button className="wa-doc-btn-voltar" onClick={zipAberto ? () => setZipAberto(null) : onClose}>
          ...
          <span className="wa-doc-header-titulo">
            {zipAberto ? zipAberto.nome : titulo}
          </span>
        </div>
      ...
      {zipAberto ? (
        <div className="wa-doc-container">
          <div className="wa-doc-secao">Conteúdo Extraído ({(zipAberto.arquivosInternos ?? []).length} itens)</div>
          <div className="wa-doc-lista">
            {(zipAberto.arquivosInternos ?? []).map((subDoc) => (
              <div key={subDoc.id} className="wa-doc-item" onClick={() => setSubDocSelecionado(subDoc)}>
                ...
                <div className="wa-doc-meta-container">
                  ...
                  <div className="wa-doc-data">{zipAberto.data}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Leitor Interno para Arquivos de dentro do ZIP */}
      {subDocSelecionado && (
        <div className="wa-reader-fullscreen sub-reader">
          <div className="wa-reader-header">
            ...
            <div className="wa-reader-header-meta">
              ...
              <span className="wa-reader-filesize">{subDocSelecionado.tipo === "png" ? "Foto" : "Documento de Texto"}</span>
            </div>
          </div>
      ...
      <style>{`
        ...
        /* ABA / DIÁLOGO FLUTUANTE DE SENHA */
        .wa-floating-overlay {
          position: fixed;
          inset: 0;
